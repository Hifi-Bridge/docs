...
<ResponseField name="transferDetails.status" type="string">
  Offramp status. `OPEN_QUOTE` means the quote is ready for review. See
  [Transaction Status](#transaction-status) for details.
</ResponseField>
...
<ResponseField name="transferDetails.receipt" type="object">
  Transaction receipt. Contains `transactionHash` once the stablecoin conversion
  is confirmed on-chain, and `paymentTracking` once fiat processing begins.
</ResponseField>

<Info>
  For detailed field documentation, see the [Create
  Offramp](https://docs.hifibridge.com/api-reference/offramp/create-an-offramp)
  API reference.
</Info>
...
<ResponseField name="transferDetails.status" type="string">
  Updated offramp status. `CRYPTO_INITIATED` means the conversion has started.
  See [Transaction Status](#transaction-status) for progression.
</ResponseField>
...
<Note>
  **Status Updates:** Subscribe to `OFFRAMP.CREATE` and `OFFRAMP.UPDATE` webhook
...
<Warning>
  **Time Sensitive:** Always check `quoteInformation.expiresAt` before
  accepting. Attempting to accept an expired quote will fail.
</Warning>
