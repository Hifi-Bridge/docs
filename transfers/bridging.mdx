...
<ResponseField name="transferDetails.id" type="string">
  Unique bridge ID. Use this to check status using the [Retrieve Bridge](https://docs.hifibridge.com/api-reference/cross-chain-bridge/retrieve-a-bridging-request) endpoint.
</ResponseField>

<ResponseField name="transferDetails.status" type="string">
  Bridge status. Progression: `CREATED` → `SOURCE_INITIATED` → `SOURCE_PROCESSED` → `DESTINATION_INITIATED` → `COMPLETED`. See [Transaction Status](#transaction-status) for details.
</ResponseField>
...
<Info>
For detailed field documentation, see the [Create Bridge](https://docs.hifibridge.com/api-reference/bridge/create-a-bridge) API reference.
</Info>
