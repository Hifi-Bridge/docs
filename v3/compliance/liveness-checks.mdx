...
<Steps>
  <Step title="Submit compliance check">
    Once the user's KYC/KYB information are updated, submit the information for compliance check.
  ...
  <Step title="Retrieve the verification link">
    If the UBO or the individual requires liveness check, the verification link will be returned under `rfiActions`with type `COMPLETE_LIVENESS_CHECK`. 
