...
| [External Card Session](#external-card-sessions) | All other clients | The recipient enters their card details on a HIFI-hosted page — raw card data never touches your servers. |
...
<Tip>
  The counterparty doesn't need to be created upfront if the card is added through an [External Card Session](https://docs.hifi.com/#external-card-sessions) submission, since the session's own schema already enforces these fields. Otherwise, you can also pass `counterpartyId` as an optional fields when create the session.
</Tip>
