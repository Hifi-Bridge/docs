...
<Steps>
  <Step title="Update Compliance Profile">
    ...
    **Response**
    ...
    ```json
    {
        "userId": "usr_pIntMywTz8LM3Y7rT8Gjd",
        "type": "BUSINESS",
        "businessName": "Random Bizz",
        "businessDescription": "move money",
  ...
  <Step title="Add Documents for Associated Party">
    ...
    **Request**
    ...
    ```shellscript
    curl -X POST "https://sandbox.hifi.com/v3/users/usr_4o0ZDdLcfxmB6OyzUEVNq/compliance/associated-parties/ap_PafbCNYpvOBnkr1U01BYI/documents" \
      -H "Authorization: Bearer YOUR_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{
        "type": "PASSPORT",
...
<AccordionGroup>
  ...
  <Accordion title="IN_REVIEW">
    ...
    **What you can expect:**
    ...
    {/* TODO: Confirm typical KYB review timing and adjust or remove the note */}
  </Accordion>
  ...
  <Accordion title="REQUIRES_INFO">
    ...
    **Example**
    ...
    {/* TODO: Confirm UPDATE_ASSOCIATED_PARTY is a real rfiAction type and
  replace the example with real sandbox output */}
  </Accordion>
</AccordionGroup>
...
| Type | Action |
| :-- | :-- |
| `UPDATE_PROFILE` | Update the business's profile information before resubmission — a common reason is information that doesn't match the submitted documentation. |
| `UPDATE_ASSOCIATED_PARTY` | Update the associated party's information before resubmission — a common reason is ownership or identity details that don't match the submitted documentation. See [Associated Parties](/v3/core/compliance/associated-parties). |
| `UPLOAD_DOCUMENT` | Update the user's documentation before resubmission — common reasons include expired, blurry, or unqualified documents. |
| `UPDATE_QUESTIONNAIRE` | Update the user's questionnaire before resubmission |
