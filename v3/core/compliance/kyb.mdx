...
| `businessType` | Legal entity type. Options: `public_company`, `cooperative`, `corporation`, `limited_liability_company`, `limited_liability_partnership`, `sole_proprietorship`, `partnership`, `dao`, `trusts`, `nonprofit`, `government_entity`, `other` | Yes |
...
| `physicalAddress` | Business address (line 1, city, state, postal code, country) | Yes |
...
| `registeredAddress` | Registered business address (line 1, city, state, postal code, country) | Yes |
...
Each UBO or person with significant control of the business must be added as an associated party, with their own profile information and identity documentation. For the required fields and endpoint details, see [Associated Parties](https://docs.hifi.com/v3/core/compliance/associated-party).
...
<Steps>
  ...
  <Step title="Add Business Documentation">
    ...
    Add `STATE_REGISTRY`to fulfill `legalPresence`, `ownershipStructure`, and `controlStructure`requirements.

    **Request**

    ```shell
    curl -X POST "https://sandbox.hifi.com/v3/compliance/documents" \
      -H "Authorization: Bearer YOUR_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{
        "userId": "usr_8kL2MdPcfxmB6OyzXWQNr",
  ...
  <Step title="Add Associated Party">
    ...
    **Request**
    ...
    ```shell
    curl -X POST "https://sandbox.hifi.com/compliance/associated-parties" \
      -H "Authorization: Bearer YOUR_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{
        "userId": "usr_8kL2MdPcfxmB6OyzXWQNr",
    ...
    **Response**
    ...
    See [Associated Parties](https://docs.hifi.com/v3/core/compliance/associated-party) for more details
  </Step>
  <Step title="Add Document for the Associated Party">
    Upload the documentation to fulfill the associated parties `identity`  and `proofOfResidence` requirements.
  ...
  <Step title="Submit compliance information for review">
...
<AccordionGroup>
  ...
  <Accordion title="REQUIRES_INFO">
    ...
    **Breakdown**
    ...
    On top of that, there are also 3 update requirements for the associated party `apt_kF9sLm2QwXcVbN3yTzR8p` that needs updates:

    - `UPDATE_PROFILE`: `ownershipPercentage`
    - `UPLOAD_DOCUMENT`: `doc_puDSeif893FEyblK`
    - `COMPLETE_LIVENESS_CHECK`: Complete the liveness check via provided `verificationLink`
...
| Type | Action |
| :-- | :-- |
| `UPDATE_PROFILE` | Update the business's or associated parties profile information before resubmission, a common reason is information that doesn't match the submitted documentation. |
| `UPLOAD_DOCUMENT` | Update the business's or associated parties documentation before resubmission, common reasons include expired, blurry, or unqualified documents. |
| `UPDATE_QUESTIONNAIRE` | Update the business's questionnaire before resubmission |
| `COMPLETE_LIVENESS_CHECK` | Complete the required liveness check for each associated party via the provided verification link. |
...
- [Associated Parties](https://docs.hifi.com/v3/core/compliance/associated-party) - UBO and control person requirements
- [Virtual Accounts](/v3/core/orchestration/virtual-accounts) - Set up fiat deposit accounts
- [API Reference](https://docs.hifi.com/v3/api-reference/compliance/update-the-compliance-profile) - Complete endpoints