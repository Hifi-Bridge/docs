...
<Info>
  A micro-deposit never counts toward the expected deposit amount and never changes the onramp's `status` — the full amount still needs to arrive in one matching payment for the onramp to proceed.
</Info>
