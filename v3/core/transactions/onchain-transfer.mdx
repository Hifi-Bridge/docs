...
Similar to a single transfer, but `destinations` takes multiple entries, each with its own `amount`. The whole batch settles as a single on-chain transaction with one `receipt`.
