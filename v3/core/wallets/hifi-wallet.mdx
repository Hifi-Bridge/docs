...
```shell
curl -X POST https://sandbox.hifi.com/v3/wallets \
  -H "Idempotency-Key: <unique-key>" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -d '{ "userId": "usr_x1y2z3", "chains": ["POLYGON", "ETHEREUM", "TRON"] }'
```
...
```shell
curl -X GET https://sandbox.hifi.com/v3/wallets/wlt_fkc0E4OsfHWZPNXiPhj4q/balance?currency=USDC \
  -H "Authorization: Bearer YOUR_API_KEY" \
```
...
```shellscript
curl -X GET https://sandbox.hifi.com/v3/wallets/wlt_ETUnJO1R8j9hDlthRnqNp/deposits?currency=USDT \
  -H "Authorization: Bearer YOUR_API_KEY" \
```
