...
<Tip>
  Virtual accounts accept both first-party and third-party deposits.
</Tip>
...
<Steps>
  ...
  <Step title="Stablecoin lands in the destination wallet">
    Once the Onramp completes, the destination wallet's balance updates. Subscribe to `ONRAMP.STATUS.*` [webhooks](https://docs.hifi.com/v3/core/webhooks/webhooks#virtual-account) to track an individual deposit's progress instead of polling.
  </Step>
</Steps>
...
<Tip>
  For supported pairs check out [Virtual Account Routing](https://docs.hifi.com/v3/core/eligibility/virtual-account-routes).
</Tip>
...
```shell
curl -X POST https://sandbox.hifi.com/v3/virtual-accounts \
...
```shell
curl -X DELETE https://sandbox.hifi.com/v3/virtual-accounts/va_HtgURm58uWMF0EzmG8DAl \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```shell
curl -X POST https://sandbox.hifi.com/v3/virtual-accounts/va_HtgURm58uWMF0EzmG8DAl/simulate-deposits \
  -H "Idempotency-Key: <unique-key>" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -d '{
    "amount": 500,
...
<AccordionGroup>
  <Accordion title="Settlement Rules">
    ...
    Check out [Settlement Rules](https://docs.hifi.com/v3/features/settlement-rule) for more information.
  </Accordion>
  ...
  <Accordion title="Routing Eligibility">
    ...
    Check out [Virtual Account Routing](https://docs.hifi.com/v3/core/eligibility/virtual-account-routes) for more information.
  </Accordion>
</AccordionGroup>
