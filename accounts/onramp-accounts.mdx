...
<Warning>
  **KYC Required:** Users must complete KYC verification for the relevant rail before creating onramp accounts. For example, to create a USD onramp account, the user must have an active USD rail. For supported African onramp corridors, the user must have completed KYC for the corresponding requirements (see [KYC Requirements](/docs/kyc/kyc-requirements)).
</Warning>
...
Onramp accounts with `type: "africa"` support multiple countries with bank accounts and mobile money (momo) as the source of fiat for onramps. **For Africa currencies, a registered onramp account is required** before creating an onramp—you must pass `source.accountId` when calling Create Onramp.
...
<ResponseField name="africa.financialInstitute" type="string" required>
  Financial institution name (e.g., `MTN`, `Vodafone`, or specific bank names). See [supported institutions](/docs/references/africa-institutions) for valid combinations.
</ResponseField>
...
<Note>
  Check [Coverage](/docs/coverage) and [supported institutions](/docs/references/africa-institutions) for onramp-supported combinations of country, currency, and financial institution.
</Note>
