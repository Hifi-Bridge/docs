...
3. When the user accepts, generate and accept the signed agreement ID in the background
4. Create the user account, passing the accepted agreement ID as `signedAgreementId`
...
<Steps>
  ...
  <Step title="Create User">
    Create User with the Signed Agreement ID
...
<AccordionGroup>
  ...
  <Accordion title="ToS Link Expiration">
    ToS links do not expire. However, the acceptance is only valid when the user completes the flow. If a user doesn't accept within your onboarding session, generate a new link with a new UUID.
  </Accordion>
