...
## Virtual Account Eligibility

Business users incorporated in India, Nigeria, Panama, the Philippines, Saudi Arabia, and Vietnam can be enabled for USD virtual accounts after every UBO with 10% ownership or above completes the required identity and liveness checks.

<Info>
  This eligibility applies to virtual accounts only. It does not expand USD
  SWIFT payout coverage for businesses incorporated in these countries. See
  [Liveness Checks](/compliance/liveness-checks) for the complete requirements.
</Info>
...
<Note>
  For businesses seeking virtual-account enablement that are subject to
  [liveness checks](/compliance/liveness-checks),
  provide every UBO with 10% ownership or above. Those UBOs must complete full
  individual KYC through the hosted verification flow, including live ID
