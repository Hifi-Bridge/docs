...
Use the USD Rail for domestic US payment methods and USD SWIFT payouts from US-based senders to the countries below.
...
<Warning>
  **Enhanced due diligence:** Countries marked with an asterisk (\*) require
  liveness checks for UBOs.
</Warning>

<Info>
  In these countries, we currently support business users only.
</Info>
...
<Note>
  For businesses subject to [liveness checks](/compliance/liveness-checks),
