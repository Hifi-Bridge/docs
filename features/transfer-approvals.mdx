...
Retrieve all transfers awaiting approval using the [List Transfer Approvals](https://docs.hifibridge.com/api-reference/transfer-approval/list-transfer-approvals) endpoint.
...
<Warning>
  **Expiration Times:** Transfers in `PENDING_APPROVAL` status will automatically expire if not approved or rejected within the following timeframes:
  - **Onchain transfers** (wallet transfers, batch transfers, bridging): Expire after **1 week**
  - **Offramps**: Expire after **1 day**
  
  Expired transfers are automatically cancelled and cannot be executed.
</Warning>
