...
**Transaction approvals** provide multi-party authorization for transactions, adding security and compliance controls to your organization's transaction workflow. Require approval before executing high-value or sensitive transactions.

<Info>
  Transaction approvals apply to on-chain stablecoin transfers (wallet transfers,
  batch transfers, and bridging) and offramps. Onramps do not support
  approval workflows.
</Info>
...
Transaction approvals work differently depending on where transactions are initiated:
...
## Creating Transactions with Approval
...
<Info>
  **Bridging and Offramp Note:** For bridging transfers and offramps, quotes are generated **after**
  approval. This prevents quotes from expiring while awaiting review.
</Info>

## Listing Pending Approvals

Retrieve all transactions awaiting approval using the [List Transaction Approvals](https://docs.hifibridge.com/api-reference/transfer-approval/list-transfer-approvals) endpoint.
...
Approve pending transfers using the [Approve Transfer](https://docs.hifibridge.com/api-reference/transfer-approval/approve-transfer) endpoint.
...
Reject pending transfers using the [Reject Transfer](https://docs.hifibridge.com/api-reference/transfer-approval/reject-transfer) endpoint.
...
<Warning>
  **Expiration Times:** Transactions in `PENDING_APPROVAL` status will automatically expire if not approved or rejected within the following timeframes:
