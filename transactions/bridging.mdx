---
title: "Bridging"
description: "Bridging transfers stablecoins across different blockchain networks. Move assets from one chain to another, such as transferring USDC from Ethereum to Polygon or Solana."
---
...
Use the [Create Bridge](https://docs.hifibridge.com/api-reference/bridge/create-a-bridge) endpoint to initiate a cross-chain transfer.
...
<ResponseField name="transferDetails.id" type="string">
  Unique bridge ID. Use this to check status using the [Retrieve Bridge](https://docs.hifibridge.com/api-reference/cross-chain-bridge/retrieve-a-bridging-request)
  endpoint.
</ResponseField>

<ResponseField name="transferDetails.status" type="string">
  Bridge status. Progression: `CREATED` → `SOURCE_INITIATED` →
  `SOURCE_PROCESSED` → `DESTINATION_INITIATED` → `COMPLETED`. See [Transaction Status](#transaction-status) for details.
</ResponseField>
...
<Info>
  For detailed field documentation, see the [Create Bridge](https://docs.hifibridge.com/api-reference/bridge/create-a-bridge) API
  reference.
</Info>
...
Monitor bridge status using the [Retrieve Bridge](https://docs.hifibridge.com/api-reference/bridge/retrieve-a-bridge) endpoint.
...
<Steps>
  ...
  <Step title="Monitor progress">
    ...
    Track through statuses: 1. SOURCE_INITIATED → SOURCE_PENDING →
    SOURCE_PROCESSED 2. DESTINATION_INITIATED → DESTINATION_PENDING → COMPLETED
  </Step>
  <Step title="Verify on block explorers">
    Once COMPLETED, use transaction hashes from `receipt.operations` to verify: Check burn transaction on source chain explorer (Polygonscan); check mint transaction on destination chain explorer (Etherscan).
  </Step>
</Steps>
...
- [API Reference](https://docs.hifibridge.com/api-reference/bridge/create-a-bridge) - Complete endpoint documentation
