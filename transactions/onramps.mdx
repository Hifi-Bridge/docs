---
title: "Onramps"
description: "Onramps convert fiat currency into stablecoins and deliver them to wallets. Users can onramp through virtual accounts (reusable) or the Onramp API (one-time virtual accounts)."
---
...
When you create an onramp, HIFI generates a **one-time account** (unique deposit details) for that transaction. For **Africa** you must register an onramp account first and pass `source.accountId`; for **USD** you typically omit it and use the one-time deposit instructions in the response. See [Onramp Accounts](/docs/accounts/onramp-accounts) for details.
...
- [Onramp Accounts](/docs/accounts/onramp-accounts) - One-time generated accounts per transaction; registered accounts for Africa
