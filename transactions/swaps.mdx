...
<ResponseField name="transferDetails.status" type="string">
  Swap status. Swaps are created with `OPEN_QUOTE` status and must be accepted before execution. See [Transaction Status](#transaction-status) for details.
</ResponseField>
...
<Note>
  **Status Updates:** Subscribe to `SWAP.TRANSACTION.CREATE` and `SWAP.STATUS.*` webhook events to receive real-time status notifications. See [Webhooks](/docs/webhooks) for setup instructions.
</Note>
