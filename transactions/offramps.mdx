...
After reviewing the quote, accept it to execute the conversion using the [Accept Quote](https://docs.hifi.com/api-reference/offramp/accept-an-offramp-quote) endpoint.
...
| **CRYPTO_RETURNED**  | Crypto returned on chain to the sender's wallet         |
...
| **FIAT_RETURNED**  | Fiat leg reversed, crypto return not yet delivered |
...
Returns are available on USD offramps and global network offramps. To test them in sandbox, see [Offramp magic values](/features/magic-values/offramp).
