...
<AccordionGroup>
  ...
  <Accordion title="KYC-Verified Users">
    **All unverified capabilities, plus:**
    ...
    - ✅ Access to regulated fiat payment capabilities
...
To enable fiat payments, users must complete KYC (Know Your Customer) verification. KYC requirements include:
...
Once approved, the user can access fiat onramp and offramp capabilities.

**Check KYC status:**

```bash
curl -X GET "https://sandbox.hifibridge.com/v2/users/usr_abc123/kyc/status" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
<AccordionGroup>
  ...
  <Accordion title="Check KYC status">
    ```bash
    curl -X GET "https://sandbox.hifibridge.com/v2/users/usr_abc123/kyc/status" \
      -H "Authorization: Bearer YOUR_API_KEY"
    ```
...
- [API Reference: Users](https://docs.hifibridge.com/api-reference/user/create-a-user) - Complete endpoint documentation
