---
title: "HIFI Wallet"
description: "Provision custodial, multi-chain wallets for a user and read their on-chain balances."
---

A HIFI Wallet is a custodial wallet HIFI provisions and secures on behalf of a user, one per chain. It's the on-chain balance a user's stablecoins live in — funded by [deposits](/v3/core/wallets/balances-deposits) or onramps, and drawn down by offramps or on-chain transfers.
...
- **Crypto Transfer** - send stablecoins on-chain to another HIFI wallet or an [external wallet](/v3/core/wallets/external-wallet), including batched transfers to many recipients at once.
...
Check out [Supported Onchain Assets](/v3/docs/references/onchain-assets) for the chains you can create for HIFI Wallet.
...
## Retrieve wallet's balance

Reads the _live_ on-chain balance for a currency - useful right before quoting or initiating an transaction, to confirm funds have actually settled.

### Example: Retrieving USDC balance

Passing `currency=USDC` as the query params to retrieve USDC balance
...
## Retrieve wallet's deposit history

Get a list of incoming deposit for supported asset for a given wallet
...
- [Onchain Asset Support](/v3/docs/references/onchain-assets)