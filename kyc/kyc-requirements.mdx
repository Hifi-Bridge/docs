...
Document groups specify minimum counts and accepted document types. Each group must be satisfied with the required number of documents from the accepted types list.

<Card title="KYC Documents Guide" href="/kyc/kyc-documents" arrow>
  Learn about document requirements for individuals, businesses, and UBOs
</Card>
...
<Tabs>
  <Tab title="Business">
    ...
    **What to watch for**
    ...
    - **Ultimate Beneficial Owners (UBOs)**: Each UBO must meet the same address, identity, and questionnaire requirements as an individual. If any UBO document is missing, the requirement appears under `invalid.documents.groups`. See the [KYC Documents guide](/kyc/kyc-documents) for UBO document requirements.
    - **Document groups**: All required groups must satisfy `minCount`. The response repeats accepted document types, so you can render them directly in an operator dashboard. See the [KYC Documents guide](/kyc/kyc-documents) for details on business document groups.
  ...
  <Tab title="Individual">
    ...
    **What to watch for**
    ...
    - **Identity documents**: Individuals must always satisfy the `identity` document group. If `minCount` is `1`, at least one document from the accepted list must be present. See the [KYC Documents guide](/kyc/kyc-documents) for individual document requirements.
...
- **[KYC Documents](/kyc/kyc-documents)** - Understand document requirements and how to upload documents for individuals, businesses, and UBOs
...
- Use the [KYC Status guide](/kyc/kyc-status) to interpret post-submission results and remediation steps
