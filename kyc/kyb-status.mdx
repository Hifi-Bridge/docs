...
To check where a business's verification stands, diagnose issues, and confirm when they're ready to use fiat payment functionalities, use the [KYC Status endpoint](https://docs.hifibridge.com/api-reference/kyc/retrieve-kyc-status). This guide focuses on interpreting the business-specific response.
...
| **ACTIVE**          | Business passed compliance review and can use fiat payment functionalities         |
...
<Tabs>
  <Tab title="ACTIVE">
    Business passed all compliance checks and completed bank onboarding. They can now use fiat payment functionalities.
  ...
  <Tab title="INACTIVE">
    ...
    {
      "status": "INACTIVE",
      "message": "KYB verification is not active.",
      "reviewResult": {},
      "details": {}
    }
    ```
  ...
  <Tab title="REJECTED">
    Business is permanently denied. Resubmission via API is not permitted.
