...
```bash
curl -X GET "https://sandbox.hifi.com/v2/users/usr_abc123/kyc/status?rails=USD" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
<Accordion title="Rails">
  ...
  <Accordion title="User Types and Business Logic">
    **Individual users** represent natural persons and have simpler KYC requirements. **Business users** represent legal entities and require additional documentation like articles of incorporation, beneficial ownership information, and authorized signer verification.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="1. Generate ToS link">
    ```bash
  ...
  <Accordion title="2. Create user (after ToS accepted)">
    ```bash
    curl -X POST "https://sandbox.hifi.com/v2/users" \
      -H "Authorization: Bearer YOUR_API_KEY" \
      -H "Content-Type: application/json" \
  ...
  <Accordion title="3. Complete KYC for USD rail">
    <Steps>
      <Step title="Update KYC information">
        Provide required personal information:
        ```bash
        curl -X POST "https://sandbox.hifi.com/v2/users/usr_abc123/kyc" \
          -H "Authorization: Bearer YOUR_API_KEY" \
          -H "Content-Type: application/json" \
          -d '{
            "phone": "+14155551234",
      ...
      <Step title="Upload identity documents">
        ...
        ```bash
        curl -X POST "https://sandbox.hifi.com/v2/users/usr_abc123/kyc/documents" \
          -H "Authorization: Bearer YOUR_API_KEY" \
          -H "Content-Type: application/json" \
          -d '[
            {
      ...
      <Step title="Submit KYC application">
        ...
        ```bash
        curl -X POST "https://sandbox.hifi.com/v2/users/usr_abc123/kyc/submissions" \
          -H "Authorization: Bearer YOUR_API_KEY" \
          -H "Content-Type: application/json" \
          -d '{"rails": "USD"}'
        ```
