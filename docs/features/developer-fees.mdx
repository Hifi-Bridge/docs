...
<Info>
Looking to charge fees for virtual accounts? Visit [Settlement Rules](/docs/features/settlement-rules) for more details.
</Info>
...
## Creating Offramps with Developer Fees
...
- **developerFee.fees** (optional): Array of fee configurations. Each fee requires:
  - **type** (required): Either `PERCENTAGE` or `FLAT`
  - **value** (required): Fee value as a string. For percentage, use decimal format (e.g., `"0.002"` for 0.2%)
  - **walletAddress** (required): Beneficiary wallet address to receive the fee

**Request Fields (standard offramp):**

- **requestId** (required): Unique identifier for the offramp request
- **source** (required): Source wallet details including userId, amount, currency, and chain
- **destination** (required): Destination account details including currency, userId, and accountId
...
<AccordionGroup>
  ...
  <Accordion title="Important Considerations">
    ...
    Best practices:
    - Verify offramp status is `COMPLETED` before releasing fee holds
    - Monitor the `transactionHash` to confirm fee distribution
    - Implement escrow mechanisms for production use
    - Test fee calculations thoroughly in sandbox before going live
  </Accordion>
</AccordionGroup>
...
- [Offramps](/docs/transfers/offramps) - Complete guide to creating offramp transactions
