...
```bash
curl --request POST \
     --url https://production.hifi.com/v2/virtual-accounts/settlement-rules \
...
```bash
curl --request POST \
     --url https://production.hifi.com/v2/users/{userId}/virtual-accounts/{accountId}/settlement-rules/{ruleId} \
     --header 'accept: application/json'
```
