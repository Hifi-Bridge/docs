...
Create a settlement rule using the [Create Settlement Rule](/api-reference/virtual-account/create-settlement-rule) endpoint.
...
- **chain** (required): Blockchain network for the settlement rule (e.g., `BASE`, `POLYGON`, `ETHEREUM`)
- **includeHIFIFee** (optional): Whether to include HIFI platform fee in the calculation. Defaults to `false`.
- **rules** (required): Array of fee rules. Each rule requires:
  - **type** (required): Either `PERCENTAGE` or `FIXED`
  - **calculationModel** (required): Either `FIXED` or `TIERED`
  - **value** (required for FIXED model): Fee value (percentage as decimal or flat amount)
  - **tiers** (required for TIERED model): Array of tier configurations
  - **walletAddress** (required): Beneficiary wallet address to receive the fee
...
<Info>
For detailed field documentation, see the [Create Settlement Rule](/api-reference/virtual-account/create-settlement-rule) API reference.
</Info>
...
After creating a settlement rule, associate it with a specific user's virtual account using the [Apply Settlement Rule](/api-reference/virtual-account/apply-settlement-rule) endpoint.
...
<AccordionGroup>
  ...
  <Accordion title="HIFI Fee Inclusion">
    ...
    - Transaction: $1,000
    - HIFI fee: $10 (1%)
    - Your fee (0.1%): Calculated on $1,010 = $1.01
  </Accordion>
...
- [API Reference](/api-reference/virtual-account/create-settlement-rule) - Complete endpoint documentation