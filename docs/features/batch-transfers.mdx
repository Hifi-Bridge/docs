...
Here's a complete payroll implementation using batch transfers:

```javascript
// Step 1: Prepare recipient list
const payrollRecipients = [
  { userId: 'usr_employee1', amount: '2500' },
  { userId: 'usr_employee2', amount: '3000' },
  { userId: 'usr_employee3', amount: '2750' },
  // ... up to 50 recipients
];
...
const wallet = await fetchWallet('usr_treasury');
if (wallet.balance < totalAmount) {
  throw new Error('Insufficient balance for payroll');
}

// Step 3: Create batch transfer
const batchTransfer = await fetch('https://sandbox.hifi.com/v2/wallets/transfers/batches', {
  method: 'POST',
  headers: {
  ...
  body: JSON.stringify({
    ...
    requireApproval: true // Payroll requires admin approval
  })
}).then(r => r.json());

console.log('Batch created:', batchTransfer.transferDetails.id);
console.log('Status:', batchTransfer.transferDetails.status);

// Step 4: Monitor via webhook
function handleWebhook(event) {
  if (event.eventType === 'WALLET.TRANSFER.BATCH.UPDATE') {
    const { id, status } = event.data;
    
    if (status === 'COMPLETED') {
      console.log(`Payroll batch ${id} completed successfully`);
      notifyEmployees(id);
    } else if (status === 'FAILED') {
      console.error(`Payroll batch ${id} failed`);
      alertFinanceTeam(id);
    }
  }
}
...
async function verifyPayroll(batchId) {
  const batch = await fetchBatchTransfer(batchId);
  
  if (batch.status === 'COMPLETED') {
    const txHash = batch.receipt.transactionHash;
    console.log(`View on Polygonscan: https://polygonscan.com/tx/${txHash}`);
    
    // All recipients received funds in this single transaction
    return true;
  }
