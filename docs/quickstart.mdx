...
In this guide, we'll show you how to use our **Sandbox API** to simulate money movement. You'll gain a clear understanding of how our endpoints work and how to integrate them into your application.

<Note>
  Before you start, get your API keys from the Dashboard to authenticate
  requests. Learn how to [get API keys](/docs/authentication).
</Note>
...
<Steps>
  <Step title="Accept Terms of Service">
    Generate a Terms of Service link and have the user sign it. You will receive
    a signed agreement ID upon acceptance.
  </Step>
  <Step title="Create User">
    Use the signed agreement ID along with the user's basic information to
    create the user on HIFI.
  </Step>
</Steps>
...
To get HIFI's Terms of Service, you can call the [Generate Terms of Service Link](https://docs.hifibridge.com/v2/reference/post_v2-tos-link) endpoint. You'll need to pass in an `idempotencyKey`, which can be any [UUID](https://www.uuidgenerator.net/). This `idempotencyKey` will be used as your signed agreement ID.
...
```curl JSON
{
  "url": "https://dashboard.hifi.com/accept-terms-of-service?sessionToken=536bb03a-8ac9-4ba7-b928-461007ecf6eb&templateId=2fb2da24-472a-4e5b-b160-038d9dc82a40&sandbox=true",
  "signedAgreementId": "8cb49537-bcf9-41b1-8d8c-c9c200d7341b"
}
```

You'll get a response object containing the following fields:

<ResponseField name="url" type="string">
  The URL directing you to HIFI's Terms of Service page. You can customize this
  page with your logo and text using a templateId.
</ResponseField>

<ResponseField name="signedAgreementId" type="string">
  The signed agreement ID, which is the same as the `idempotencyKey` you passed
  in. This will be valid only after the user accepts the Terms of Service.
</ResponseField>
...
Now that we have a valid `signedAgreementId`, we can create a user by calling the [Create User](/api-reference/user/create-a-user) endpoint. We've provided all the basic personal information with dummy values in the curl request below.
...
Congrats! We've successfully created a user with the id: `32051b2f-0798-55a7-9c42-b08da4192c97`. Let's take a look at the response.

<ResponseField name="id" type="string" required>
  The user ID, which should be saved for future API calls for this particular
  user.
</ResponseField>

<ResponseField name="wallets" type="object" required>
  Contains all the wallet types and addresses provisioned for the user.
</ResponseField>

<Info>
  Once a user is created, HIFI automatically assigns them wallet addresses so
  you can start moving money onchain.
</Info>

## KYC

To move money between stablecoins and fiat, you need to provide KYC for the user and unlock a **Rail**. Each rail enables a different set of currencies. In this quickstart, we'll focus on USD.

<Note>
  The KYC information requested depends on the rail. Visit our{" "}
  <a href="/rails">Rails page</a> to learn more about available options.
</Note>

<Steps>
  <Step title="Gather KYC Requirements">
    Consult the documentation or use the Retrieve KYC Requirements endpoint to
    identify the required information.
  </Step>
  <Step title="Update KYC Information">
    Provide required fields using the Update KYC endpoint.
  </Step>
  <Step title="Upload KYC Documents">
    Upload necessary documents using the Add Documents endpoint.
  </Step>
  <Step title="Submit KYC">
    Once all requirements have been fulfilled, call the Submit KYC endpoint to
    unlock the rail.
  </Step>
  <Step title="Check KYC Status">
    Use the Retrieve KYC Status endpoint to check for updates.
  </Step>
</Steps>
...
First, we need to get the KYC requirements for our user by calling the Retrieve KYC Requirements endpoint with the `userId`.
...
<ResponseField name="required" type="object" required>
  All the mandatory KYC fields needed for the USD rail KYC application.
</ResponseField>

<ResponseField name="optional" type="object" required>
  Additional KYC fields that are not mandatory but may be provided.
</ResponseField>

<ResponseField name="invalid" type="object" required>
  Any fields that must be corrected before the KYC application can proceed. From
  the response, we can see that the user still has multiple missing KYC fields.
</ResponseField>

Now that we know the missing KYC fields, let's update the user's KYC information.

### Update KYC Information

To submit required KYC information, you can use the [Update KYC](https://docs.hifibridge.com/v2/reference/post_v2-users-userid-kyc) endpoint. We've provided dummy values in the curl request.
...
Users can upload raw documentation directly via the Upload a File endpoint. HIFI will return a file ID, which can later be used to attach the document to the user.
...
<ResponseField name="id" type="string" required>
  The file ID, which will be used to attach the document to the user.
</ResponseField>
...
Now that we have the `fileId`, we can attach the documentation to the user. In this example, we will use the previously uploaded file as the user's driver's license to satisfy the `identity` requirement.
...
<ResponseField name="data" type="array" required>
  An array of document objects, each containing the document details including
  id, type, subType, issuedCountry, url, and fileId.
</ResponseField>
...
To submit the KYC information the user currently holds for the **USD** rails, we can use the [Submit KYC](/api-reference/kyc/submit-kyc) endpoint.

> 📘 The Submit KYC endpoint submits the existing KYC information stored for the user to the specified rail. If you want to submit any new KYC data that the user doesn't currently hold, you must first update the user's KYC information using the [Update KYC](/api-reference/kyc/update-kyc-information) endpoint before calling [Submit KYC](/api-reference/kyc/submit-kyc).

Let's submit the user's KYC information to unlock the **USD** rails. This can be done by calling the [Submit KYC](/api-reference/kyc/submit-kyc) endpoint and providing the `userId` and the `rails`.
...
After submission, the KYC status will initially be set to `"CREATED"`. You can either call the [Retrieve KYC Status](https://docs.hifibridge.com/v2/reference/get_v2-users-userid-kyc-status) endpoint or wait for [webhook](https://docs.hifibridge.com/v2/docs/webhooks) events to receive updates on the user's latest KYC status for that rail.

### Retrieve KYC Status

To get the KYC status for a specific rails, the user can call the [Retrieve KYC Status](/api-reference/kyc/retrieve-kyc-status) endpoint.
...
Let's make a [Create a virtual account](/api-reference/virtual-account/create-a-virtual-account) call using the user id we created earlier, with the parameters we just mentioned:
...
To offramp, you can add a USD offramp bank account by making an [Create a USD Offramp Bank Account](/api-reference/account/create-an-account) call.

Let's add a USD offramp bank account for Wire. To do this, you'll need to provide your bank account details. However, for the purpose of this guide, we've pre-configured the bank account details for you, so all you need to do is call the [Create a USD Offramp Bank Account](/api-reference/account/create-an-account) endpoint:
...
In production, the entire flow is only triggered after the user makes a deposit and we confirm that the fiat has landed. In the sandbox, we provide a simulation endpoint to mimic the behavior of an incoming deposit.
...
After the onramp transfer is completed, User A will have 1 USDC in their wallet. Now we can transfer the 1 USDC from User A's wallet to User B's wallet. To do this, we can call the [Create a crypto transfer](/api-reference/crypto-transfer/create-a-crypto-transfer) endpoint.
...
- The `id` is a unique identifier for this transfer, which you can save to get the most up-to-date transfer status using the [Retrieve a crypto transfer](/api-reference/crypto-transfer/retrieve-a-crypto-transfer) endpoint.
- The `status` represents the transfer status, which initially appears as `"CREATED"`. You will want to either register a webhook or poll the [Retrieve a crypto transfer](/api-reference/crypto-transfer/retrieve-a-crypto-transfer) endpoint to monitor the latest transfer status until the transfer is `"COMPLETED"`.
...
Now let's offramp the rest of the 1 USDC to userA's bank account. To do this, we can call the [Create an offramp](/api-reference/offramp/create-an-offramp) endpoint.
...
- The `id` is a unique identifier for this transfer, which you can save to get the most up-to-date transfer status using the [Retrieve an offramp](/api-reference/offramp/retrieve-an-offramp) endpoint.
...
After confirming the quote, call the Accept Quote Endpoint to start the rest of the offramp process before the quote expires, as indicated by the `expiresAt` field.
