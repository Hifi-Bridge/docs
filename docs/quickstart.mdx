...
In this guide, you'll learn how to use the **HIFI Sandbox API** to simulate money movement. You'll create users, complete KYC, set up accounts, and execute a series of transfer flows between fiat and stablecoins.

**⏱️ Time to complete:** 20-30 minutes

**🎯 What you'll build:** A complete transfer flow where User A onramps $2 USD to USDC, transfers 1 USDC to User B, then offramps the remaining 1 USDC back to fiat.
...
- ✅ KYC applications are typically automatically approved (may be blocked by compliance checks)
...
First, find out what information is needed for the USD rail by calling the [Retrieve KYC Requirements](https://docs.hifibridge.com/v2/reference/get_v2-users-userid-kyc-requirements) endpoint.
...
Provide the missing personal information using the [Update KYC](https://docs.hifibridge.com/v2/reference/post_v2-users-userid-kyc) endpoint.
...
First, upload the identity document (driver's license, passport, etc.) using the [Upload a File](https://docs.hifibridge.com/v2/reference/post_v2-files) endpoint.
...
Now attach the uploaded file(s) to the user's KYC application using the [Add Documents](https://docs.hifibridge.com/v2/reference/post_v2-users-userid-kyc-documents) endpoint.
...
With all fields and documents provided, submit the KYC application to unlock the USD rail using the [Submit KYC](https://docs.hifibridge.com/api-reference/kyc/submit-kyc) endpoint.
...
1. **Poll** the [Retrieve KYC Status](https://docs.hifibridge.com/api-reference/kyc/retrieve-kyc-status)endpoint
...
Check the current KYC status for the USD rail using the [Retrieve KYC Status](https://docs.hifibridge.com/api-reference/kyc/retrieve-kyc-status) endpoint.
...
Now User A will send 1 USDC to User B using the [Create a crypto transfer](https://docs.hifibridge.com/api-reference/crypto-transfer/create-a-crypto-transfer) endpoint.
