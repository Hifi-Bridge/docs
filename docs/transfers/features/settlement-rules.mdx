...
<AccordionGroup>
  ...
  <Accordion title="Tiered Calculation with Percentage">
    ...
    <Info>
      For tiered rules, leave `min` empty for the first tier and `max` empty for the
      last tier to create open-ended ranges.
    </Info>
...
2. Each rule is evaluated in order to calculate fees
