...
Exchange one cryptocurrency for another on the same blockchain network.
...
<ResponseField name="transferDetails.id" type="string">
  Unique swap ID. Use this to check status using the [Retrieve Swap](/api-reference/swap/retrieve-a-swap) endpoint.
</ResponseField>
