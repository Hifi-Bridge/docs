...
Create an onramp to get a conversion quote using the [Create Onramp](/api-reference/onramp/create-an-onramp) endpoint.

**Request:**

```bash
curl -X POST "https://sandbox.hifi.com/v2/onramps" \
...
<ResponseField name="source" type="object" required>
  Source of the onramp (fiat side).
  
  <Expandable title="properties">
    <ResponseField name="userId" type="string" required>
      User initiating the onramp
    </ResponseField>
    <ResponseField name="currency" type="string" required>
      Fiat currency to convert from (e.g., `usd`, `eur`, `brl`)
    </ResponseField>
    <ResponseField name="amount" type="number" required>
      Amount of fiat to convert
    </ResponseField>
    <ResponseField name="transferType" type="string" required>
      Payment method: `ach`, `wire`, `pix`, `spei`, etc.
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="destination" type="object" required>
  Destination of the onramp (crypto side).
  
  <Expandable title="properties">
    <ResponseField name="userId" type="string" required>
      User receiving the stablecoins
    </ResponseField>
    <ResponseField name="currency" type="string" required>
      Stablecoin to receive (e.g., `usdc`, `usdt`)
    </ResponseField>
...
<ResponseField name="requestId" type="string" required>
  Unique identifier (UUID) for idempotency
</ResponseField>
...
<Note>
**Status Updates:** Subscribe to `ONRAMP.CREATE` and `ONRAMP.UPDATE` webhook events to receive real-time status notifications. See [Webhooks](/docs/webhooks) for setup instructions.
</Note>

Monitor onramp status using the [Retrieve Onramp](/api-reference/onramp/retrieve-an-onramp) endpoint. The response structure matches the create endpoint shown above.

**Request:**

```bash
curl -X GET "https://sandbox.hifi.com/v2/onramps/onr_abc123" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
Quotes typically expire within **24 hours** of creation. This ensures:
...
<Info>
**Quote Expiration:** The `expiresAt` timestamp in `quoteInformation` shows exactly when the quote expires. Plan to send your deposit with enough time for bank processing before this deadline.
</Info>
...
<AccordionGroup>
  ...
  <Accordion title="Exact Amount Matching">
    ...
    - Include all decimal places (e.g., $100.00, not $100)
...
- [API Reference](/api-reference/onramp/create-an-onramp) - Complete endpoint documentation