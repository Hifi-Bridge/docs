...
Currently, we support the following onramp methods:

- **ACH transfers** for USD to USDC conversions
...
1. **Create an onramp** – Initiate an onramp request with your source fiat currency and destination stablecoin
2. **Review quote** – Examine the exchange rate, fees, and quote validity period
3. **Deposit funds** – Transfer the fiat currency to the provided bank account using the reference ID
...
Onramps may include rail fees for processing fiat transfers, which are clearly displayed in the quote information. The fee structure is transparent and calculated based on current market conditions.

### Fee Structure

- **Rail fees**: Fees charged by the payment rail (ACH, wire, etc.)
- **Exchange fees**: Fees for converting fiat to stablecoin (if applicable)
