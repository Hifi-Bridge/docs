...
This guide shows you how to submit KYC (Know Your Customer) information programmatically using the HIFI API. Users must complete KYC verification to enable currency conversions between fiat and stablecoins.
...
1. **Retrieve KYC Requirements** - Get required information for KYC verification
...
First, find out what information is needed for KYC verification by calling the [Retrieve KYC Requirements](https://docs.hifibridge.com/v2/reference/get_v2-users-userid-kyc-requirements) endpoint.

**Request:**

```bash
curl --request GET \
     --url 'https://sandbox.hifibridge.com/v2/users/32051b2f-0798-55a7-9c42-b08da4192c97/kyc/requirements' \
...
<ResponseField name="required" type="object">
  All mandatory KYC fields needed for verification. These must be provided
  before submission.
...
With all fields and documents provided, submit the KYC application for review using the [Submit KYC](https://docs.hifibridge.com/api-reference/kyc/submit-kyc) endpoint.
...
Check the current KYC status using the [Retrieve KYC Status](https://docs.hifibridge.com/api-reference/kyc/retrieve-kyc-status) endpoint.

**Request:**

```bash
curl --request GET \
     --url 'https://sandbox.hifibridge.com/v2/users/32051b2f-0798-55a7-9c42-b08da4192c97/kyc/status' \
     --header 'accept: application/json' \
     --header 'authorization: Bearer YOUR_API_KEY'
```
...
✅ **KYC approved!** The status is `ACTIVE`, which means the user can now:
...
<ResponseField name="status" type="string">
  Overall KYC status. Possible values: - `CREATED` - Application submitted, not
  yet reviewed - `PENDING` - Being reviewed by compliance team - `ACTIVE` -
  Approved, user can use fiat payment functionalities - `REJECTED` - Not
  approved (see rejectReasons for details)
</ResponseField>
...
Now that your user has completed KYC verification, you can:
