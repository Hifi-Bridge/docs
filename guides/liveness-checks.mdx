...
description: "Show UBOs the hosted liveness check returned by the KYC Status API."
---

Some business users require their Ultimate Beneficial Owners (UBOs) to complete a liveness check before the USD rail can be activated.
...
<Steps>
  ...
  <Step title="Wait for ACTIVE">
    Continue checking the business status until it becomes `ACTIVE`.
  </Step>
</Steps>
...
Enable the USD rail only when the top-level `status` is `ACTIVE`.
