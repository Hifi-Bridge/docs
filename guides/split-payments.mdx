...
**🎯 What you'll build:** A split payment system where users deposit USD into a virtual account, funds convert to USDC, then split payments are distributed to multiple recipients through the use of batch transfers.
...
Create a batch transfer to split a payment among multiple recipients in a single transaction using the [Create Batch Transfer](https://docs.hifi.com/api-reference/crypto-transfer/create-batch-transfer) endpoint.
...
<Info>
  **Monitoring Batch Transfers:** Subscribe to `WALLET.TRANSFER.BATCH.UPDATE`
  ...
  Transfer](https://docs.hifi.com/api-reference/crypto-transfer/retrieve-batch-transfer)
