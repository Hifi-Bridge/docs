...
- Creating a user
- Submitting KYC to unlock a rail
...
To get HIFI's Terms and Service page, you can call the [Generate Terms of Service Link](/api-reference/user/generate-a-terms-of-service-link) endpoint. You will need to pass in an `idempotencyKey`, which can be any [UUID](https://www.uuidgenerator.net/). This `idempotencyKey` will be used as your signed agreement ID.
...
Now that we have the user's valid `signedAgreementId` and basic personal information, we can create a user by calling the [Create User](/api-reference/user/create-a-user) endpoint. We have provided all the basic personal information with dummy values in the curl request.
...
Let's get the KYC requirements for **USD** rails by calling the Retrieve KYC Requirements endpoint and passing in **USD** as the rails and the userId.
...
- The `required` fields represents all the mandatory KYC information needed for the **USD** rail KYC application.
- The `optional` fields represents all the additional KYC information that is not mandatory but may be provided for the **USD** rail KYC application.
...
To update the user's KYC information, you can use the [Update KYC](/api-reference/kyc/update-kyc-information) endpoint. Let's update the user's KYC information. We have provided all the needed KYC fields with dummy values in the curl request.
...
The response contains the latest KYC information the user holds after the update. This same set of information can also be retrieved using the [Retrieve KYC information](/api-reference/kyc/retrieve-kyc-information) endpoint.
...
In this example, we will use the previously uploaded file as the user's driver's license to satisfy the `identity` requirement.
...
After submission, the KYC status will initially be set to `"CREATED"`. The user can either call the [Retrieve KYC Status](/api-reference/kyc/retrieve-kyc-status) endpoint or wait for [webhook](/docs/webhooks) events to receive updates on the user's latest KYC status for that rail.
